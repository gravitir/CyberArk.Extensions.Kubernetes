    ...
    public class Reconcile : BaseAction
    {
        ...
        override public int run(ref PlatformOutput platformOutput)
        {
            ...
            try
            {
                ...
                string targetKcEncoded = TargetAccount.CurrentPassword.convertSecureStringToString();
                string reconcileKcEncoded = ReconcileAccount.CurrentPassword.convertSecureStringToString();
                ...
                string targetKcString = System.Text.Encoding.UTF8.GetString(Convert.FromBase64String(targetKcEncoded));
                dynamic targetKcDeconstructed = deserializer.Deserialize<dynamic>(targetKcString);
                string reconcileKcString = System.Text.Encoding.UTF8.GetString(Convert.FromBase64String(reconcileKcEncoded));
                dynamic reconcileKcDeconstructed = deserializer.Deserialize<dynamic>(reconcileKcString);
                ...
                Org.BouncyCastle.X509.X509Certificate bcClientCert = ReadCertificate(reconcileKcDeconstructed["users"][0]["user"]["client-certificate-data"]);
                AsymmetricKeyParameter keyParam = ReadPrvKey(reconcileKcDeconstructed["users"][0]["user"]["client-key-data"]);
                ...
                targetKcDeconstructed["users"][0]["user"]["client-certificate-data"] = responseGetCertJson["status"]["certificate"].ToString();
                targetKcDeconstructed["users"][0]["user"]["client-key-data"] = keyPairAndCsr.Item1;
                ISerializer serializer = new SerializerBuilder().Build();
                dynamic targetKcReconstructed = serializer.Serialize(targetKcDeconstructed);
